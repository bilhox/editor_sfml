...
void ui::Section::setWeights(std::vector<float> weights){
    if((std::size(weights) != std::size(m_childs)) && std::accumulate(weights.begin() , weights.end() , 0.f) > 1)
        return;
...
void ui::Section::addChild(std::shared_ptr<ui::Component> component_ptr){
    ...
    m_childs.emplace_back(std::move(component_ptr));
    ...
    if(std::size(m_weights) != 0){
        for(auto & weight : m_weights){
            weight = 1/(float)std::size(m_childs);
        }
    }
